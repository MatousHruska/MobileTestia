...
func _ready() -> void:
	...
	Debug.print_saveload("[SAVELOAD] Zone: ChunkManager state BEFORE init: initialized=%s, zone=%s" % [ChunkManager._initialized if ChunkManager else "null", ChunkManager.current_zone_id if ChunkManager else "null"])
	...
	Debug.print_saveload("[SAVELOAD] Zone: Set Game.current_zone to scene filename: %s (zone_id=%s)" % [scene_filename, zone_id])

	# Initialize chunk system for this zone
	if use_chunk_system:
		Debug.print_saveload("[SAVELOAD] Zone: Initializing ChunkManager for zone: %s" % zone_id)
		...
		if chunk_mgr:
			...
			Debug.print_saveload("[SAVELOAD] Zone: ChunkManager init done, state: initialized=%s, zone=%s" % [chunk_mgr._initialized, chunk_mgr.current_zone_id])
	...
	Debug.print_saveload("[SAVELOAD] Zone: NPCManager state: enemies=%d, spawn_points=%d" % [NPCManager.all_enemies.size() if NPCManager else 0, NPCManager.all_spawn_points.size() if NPCManager else 0])
	Debug.print_saveload("[SAVELOAD] ====== ZoneBase._ready() END | Frame: %d ======" % Engine.get_process_frames())
...
func _exit_tree() -> void:
	...
	Debug.print_saveload("[SAVELOAD] ====== ZoneBase._exit_tree() | Frame: %d ======" % Engine.get_process_frames())
	Debug.print_saveload("[SAVELOAD] Zone exit: name=%s, id=%s" % [zone_name, zone_id])
	Debug.print_saveload("[SAVELOAD] Zone exit: Game.current_state=%s" % (Game.GameState.keys()[Game.current_state] if Game else "null"))
	Debug.print_saveload("[SAVELOAD] Zone exit: ChunkManager state BEFORE cleanup: initialized=%s, zone=%s" % [ChunkManager._initialized if ChunkManager else "null", ChunkManager.current_zone_id if ChunkManager else "null"])
	Debug.print_saveload("[SAVELOAD] Zone exit: NPCManager BEFORE cleanup: enemies=%d, spawn_points=%d" % [NPCManager.all_enemies.size() if NPCManager else 0, NPCManager.all_spawn_points.size() if NPCManager else 0])

	if use_chunk_system:
		var chunk_mgr = get_node_or_null("/root/ChunkManager")
		if chunk_mgr:
			Debug.print_saveload("[SAVELOAD] Zone exit: Calling ChunkManager.cleanup_zone()")
