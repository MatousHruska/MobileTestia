...
## When direction is provided, checks for a direction-specific resource first
## (e.g., "hilt_bash_down.tres"), then non-directional, then hardcoded.
static func get_template(template_id: String, direction: String = "") -> AbilityVisualData:
	# Check for direction-specific resource (per-direction Attack Composer export)
	if not direction.is_empty():
		var dir_path := "res://resources/sequences/%s_%s.tres" % [template_id, direction]
		if ResourceLoader.exists(dir_path):
			var loaded := load(dir_path)
			if loaded is AbilityVisualData:
				return loaded
	# Check for non-directional resource
