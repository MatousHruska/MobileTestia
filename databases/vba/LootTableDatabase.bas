...
Public Sub ExportLootTables()
    ...
    For i = 2 To lastRow
        ...
        json = json & "      ""min_drops"": " & GetDefaultNumeric(ws.Cells(i, COL_LT_MIN_DROPS), 1) & "," & vbCrLf
        json = json & "      ""max_drops"": " & GetDefaultNumeric(ws.Cells(i, COL_LT_MAX_DROPS), 3) & "," & vbCrLf
        json = json & "      ""rarity_weights"": {" & vbCrLf
        json = json & "        ""nothing"": " & GetDefaultNumeric(ws.Cells(i, COL_LT_NOTHING_WEIGHT), 50) & "," & vbCrLf
        json = json & "        ""common"": " & GetDefaultNumeric(ws.Cells(i, COL_LT_COMMON_WEIGHT), 100) & "," & vbCrLf
        json = json & "        ""magic"": " & GetDefaultNumeric(ws.Cells(i, COL_LT_MAGIC_WEIGHT), 30) & "," & vbCrLf
        json = json & "        ""rare"": " & GetDefaultNumeric(ws.Cells(i, COL_LT_RARE_WEIGHT), 10) & "," & vbCrLf
        json = json & "        ""unique"": " & GetDefaultNumeric(ws.Cells(i, COL_LT_UNIQUE_WEIGHT), 1) & vbCrLf
        json = json & "      }," & vbCrLf
        json = json & "      ""gold_min"": " & GetDefaultNumeric(ws.Cells(i, COL_LT_GOLD_MIN)) & "," & vbCrLf
        json = json & "      ""gold_max"": " & GetDefaultNumeric(ws.Cells(i, COL_LT_GOLD_MAX), 10) & "," & vbCrLf
