...
Public Sub ExportSpawnPoints()
    ...
    For i = 2 To lastRow
        ...
        json = json & "      ""min_level"": " & GetDefaultNumeric(ws.Cells(i, COL_SP_MIN_LEVEL), 1) & "," & vbCrLf
        json = json & "      ""max_level"": " & GetDefaultNumeric(ws.Cells(i, COL_SP_MAX_LEVEL), 5) & "," & vbCrLf
        json = json & "      ""check_interval"": " & GetDefaultNumeric(ws.Cells(i, COL_SP_CHECK_INTERVAL), 60) & "," & vbCrLf
        json = json & "      ""spawn_chance"": " & GetDefaultNumeric(ws.Cells(i, COL_SP_SPAWN_CHANCE), 1) & "," & vbCrLf
        json = json & "      ""max_active_enemies"": " & GetDefaultNumeric(ws.Cells(i, COL_SP_MAX_ACTIVE), 1) & "," & vbCrLf
        json = json & "      ""respawn_delay"": " & GetDefaultNumeric(ws.Cells(i, COL_SP_RESPAWN_DELAY)) & "," & vbCrLf
        json = json & "      ""spawn_radius"": " & GetDefaultNumeric(ws.Cells(i, COL_SP_SPAWN_RADIUS)) & "," & vbCrLf
