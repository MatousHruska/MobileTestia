...
Fields:
  ...
  - z_mode: Enum "y_sort" | "fixed_back" | "fixed_front" (default: "y_sort")
  - shadow_mode: Enum "baked" | "realtime" (default: "baked")
```

Environmental decoration entity. Assets are generated by the **Decoration Pipeline** tool (Tools Menu).

**Asset Structure:**
```
assets/decorations/{decoration_id}/
├── sprite.png     (required)
├── normal.png     (optional — enables 2D lighting interaction)
├── shadow.png     (optional — baked shadow drawn behind sprite)
└── occluder.tres  (optional — LightOccluder2D polygon for realtime shadows)
```

**Z-Mode Options:**
- `y_sort` (z_index=0) — default Y-position sorting for natural depth
- `fixed_back` (z_index=-5) — always behind terrain and other entities
- `fixed_front` (z_index=10) — always in front (foreground elements)

**Shadow Mode Options:**
- `baked` — uses pre-rendered `shadow.png` drawn at z_index -1
- `realtime` — uses `occluder.tres` polygon with LightOccluder2D

**Multi-Angle Decorations:**
The Decoration Pipeline can generate multiple viewing angles per model, creating suffixed IDs:
- Single: `stone_pillar` (no suffix)
- Two angles: `statue_front`, `statue_back`
- Four angles: `statue_front`, `statue_back`, `statue_left`, `statue_right`

Each suffixed ID is an independent decoration folder.

**LDtk Tileset Atlas:**
The Decoration Pipeline generates `assets/decorations/_atlas/decoration_atlas.png` — a visual grid of all decorations. Register this as a tileset in LDtk to see sprite previews when placing Decoration entities.
...
+-- For each decoration:
    +-- Spawn via DecorationSpawner (always spawns, no persistence)
```
